...
/**
 * Given a master record ID and an array of IDs to Fhir objects of the same type, 
 * replace all references to the matching ID with the master ID and optionally
 * delete the matching record once done.
 * 
 */
Client.prototype.merge = function(recordType, primary, duplicate, callback){
	...
	c.getRecord(recordType,duplicate,function(error, success) {
		...
		_.forEach(success.entry, function(entry){
			...
			if (entry.resource.resourceType != 'Provenance' && entry.resource.id != duplicate 
		...
		//stringify
		var stringRep = JSON.stringify(transaction);
		...
		transaction = JSON.parse(stringRep);

		c.fhirClient.transaction(transaction,callback);
