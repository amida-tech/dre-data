	...
	c.getRecord(recordType,duplicate,function(error, success) {
		...
		_.forEach(dupEntries, function(entry){
			...
			if (entry.resource.resourceType != 'Provenance' 
				...
				mergeExtensions(p, entry.resource);
			}
		});
		
		c.fhirClient.transaction(transaction,function(err, success){
			//on completion of the update, call the deletion
...
Client.prototype.replace = function(recordType, primary, duplicate, callback){
	...
	c.getRecord(recordType,duplicate,function(error, success) {
		...
		c.fhirClient.transaction(transaction,function(err, success){
			...
			if (duplicateWithExtensions == null){
				c.fhirClient.transaction(deleteTransaction,function(deleteErr, deleteSuccess){
					callback(err, success);				
				});
				...
				c.getRecord(recordType,primary,function(error, success) {
					var originalRecord = success.entry[0];
					//this combines the extension elements at the base level for the 2 records prior to submission
					mergeExtensions(originalRecord, duplicateWithExtensions);
 ...
 */
var mergeExtensions = function(primaryRecord, secondaryRecord){
	...
	if (!_.isUndefined(secondaryRecord.extension)){
