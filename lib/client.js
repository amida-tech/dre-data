		...
		_.forEach(matchSet, function(entry){
			if ('match' == entry.changeType){
				_.forEach(entry.matches, function(match){
					c.replace(entry.lhs.resourceType, entry.lhs.id, match.id, function(s){
						//not doing anything with the replace, simply letting it execute asynchronously
//						console.log("replaced "+match.resourceType+'/'+match.id+" with "+entry.lhs.id);
					});
				});
			}else{
				resp.push(entry);
	...
	c.getRecord(recordType,duplicate,function(error, success) {
		...
		_.forEach(dupEntries, function(entry){
			...
			if (entry.resource.resourceType != 'Provenance' 
				...
				transaction.addEntry(entry.resource, 'DELETE');
			}
		});
