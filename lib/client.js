...
var isMismatch = function(original, proposed){
	if (!_.isUndefined(original.extension)&& !_.isUndefined(proposed.id)){
		...
		for (var j=0; j < original.extension.length; j++){
			...
			if (original.extension[j].url === "http://amida-tech.com/fhir/extensions/mismatch" && !_.isUndefined(original.extension[j].mismatches)){
				//see if any of the ids match the id of the proposed match
				for (var k=0; k < original.extension[j].mismatches.length; k++){
					if (proposedMatchId == original.extension[j].mismatches[k]){
