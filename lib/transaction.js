...
/**
 * NOTE: the fhir transaction is actually under this.bundle.  
...
exports.createTransaction = function(createProvenance) {
	return new Transaction(_.isUndefined(createProvenance)?false:createProvenance );
};
...
//		console.log('yes, create provenance');
		...
		for (i =0; i < entries.length; i++){
			...
			if (_.isUndefined(fhirObject.id)){
				id = rType+'/tempProvId';
				fhirObject.id = id;
			}else{
				id = fhirObject.id;
	...
	}else if (_.isUndefined(this.provenance)){
...
Transaction.prototype.addSource = function(source){
	...
	if (!_.isUndefined(this.provenance))
...
Transaction.prototype.addEntry = function(fhirObject, method, url, base){
	...
	if (_.isArray(fhirObject)){
		...
		return entryIndex;
	}else if (!_.isUndefined(fhirObject.resource)){
		...
		this.bundle.entry.push({
			...
			"base" : _.isUndefined(base)?"http://localhost:8080/fhir":base
		});
