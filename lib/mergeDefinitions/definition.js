					...
					if (matchCriteria[compiled_path].type == 'levenshtien'){
						...
						if (kind === 'D' || kind === 'N'){
							...
							if (patientRecord !== null && !_.isUndefined(patientRecord.all)){
								...
								if (!_.isUndefined(leftFldVal)&&!_.isUndefined(rightFldVal)){
...
//									console.log ('>>>>'+JSON.stringify(fldPath)+'>>'+leftRef+'>>>>>>'+leftFldVal);
//									console.log(JSON.stringify(leftObj));
//									console.log ('>>>>>>>>>>>>'+rightFldVal);
									distance = lev(leftFldVal, rightFldVal, true);
								}
